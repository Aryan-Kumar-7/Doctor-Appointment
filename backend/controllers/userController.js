...
const paymentStripe = async (req, res) => {
    ...
    try {
        ...
        // Creating a Payment Intent for the appointment
        const paymentIntent = await stripeInstance.paymentIntents.create({
            amount: appointmentData.amount * 100, // Amount in smallest currency unit
            currency: process.env.CURRENCY || 'usd', // Default to 'usd' if CURRENCY is not defined
...
const verifyPayment = async (req, res) => {
    try {
        const { paymentIntentId } = req.body; // Expecting paymentIntentId from the frontend
        ...
        if (paymentIntent.status === 'succeeded') {
            ...
            await appointmentModel.findByIdAndUpdate(paymentIntent.metadata.receipt, {payment:true})
